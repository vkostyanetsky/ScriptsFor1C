...
#
# Простые варианты решения той же задачи:
#
# grep -ho ",EXCP,.*Descr=.*" */*.log | perl -pe "s/,EXCP,.,.*Descr=/Descr=/g" | uniq -c | sort -rn
# grep -hoP ",EXCP,.*\KDescr=.*" */*.log | uniq -c | sort -rn
#
# Проблема в том, что описание у EXCP может быть многострочным — то есть, варианты выше будут терять часть данных,
# нужных для расследования проблемы. Поэтому здесь нужен несколько более сложный подход.
