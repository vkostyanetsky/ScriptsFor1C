...
#
# Каждое событие начинается с времени (например, 14:55.636001-). Сделаем такую подстроку разделитем записей.
#
# Описание ошибки, если есть, помещается в строке события последним, после подстроки «Descr=». Сделаем её разделителем полей;
# тогда каждая запись будет иметь два поля: первое — cтрока события до описания, второе — описание.
#
